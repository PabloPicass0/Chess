...
bool ChessBoard::validMove(const int source_file, const int source_rank, const int destination_file, const int destination_rank) {
  ...
  if (source_file < 0 || source_file > 7 || source_rank < 0 || source_rank > 7) {
    cout << "Your source square is out of range!\n";
    return false;
  }
  ...
  if (destination_file < 0 || destination_file > 7 || destination_rank < 0 || destination_rank > 7) {
    cout << " Your destination square is out of range!\n";
    return false;
  }
  ...
  if (pieces[source_rank][source_file] == NULL) {
    cout << "There is no piece at the source square!\n";
    return false;
  }
...
bool ChessBoard::submitMove(const char source_square[2], const char destination_square[2]) {
  ...
  if (strlen(source_square) != 2 || strlen(destination_square) != 2) {
    cout << "Please input only valid positions!\n";
    return false;
  }
  ...
  //conducts basic checks (out of range, turn etc.)
  ...
  if (pieces[source_rank][source_file]->get_name() != "Knight" && isBlocked(source_file, source_rank, destination_file, destination_rank)) {
    cout << "The move of your piece is blocked!";
    return false;
  }

  //checks if piece can move like that, if yes, make move and return true
  ...
  //if the piece can perform the movement
  //if it is check
